...
function before_v1_gate {
  ...
  stack {
    ...
    var $direct_performer {
      value = $input.provider.credential_type|in:$input.rule_snapshot.direct_performer_credentials
    }

    var $delegated_performer {
      value = $input.provider.credential_type|in:$input.rule_snapshot.delegated_path_credentials
    }

    var $interpretation_review {
      value = $input.provider.credential_type|in:$input.rule_snapshot.credential_interpretation_review
    }
    ...
    var $patient_flags {
      value = $input.encounter|get:"patient_flags":"{}"|json_decode
    }
