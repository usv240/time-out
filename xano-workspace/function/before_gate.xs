...
function before_gate {
  ...
  stack {
    ...
    var $assessment_facts {
      value = {
        practitioner_patient_relationship_established: $input.encounter.practitioner_patient_relationship_established
        adequate_medical_record_present              : $input.encounter.adequate_medical_record_present
        performer_identity_disclosed                 : $input.encounter.performer_identity_disclosed
        good_faith_exam_label_status                 : "REVIEW"
      }
    }
  
    conditional {
      if ($input.encounter.practitioner_patient_relationship_established && $input.encounter.adequate_medical_record_present && $input.encounter.performer_identity_disclosed) {
        array.push $findings {
          value = {
  ...
  guid = "xcDopDcxQ514_4g277rc8EMYxXo"
}