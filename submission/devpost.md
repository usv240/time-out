...
- **Foxit** runs the assembly agent: from a plain prompt, through the official Foxit MCP server, to a watermarked three-page record — then it stops, and the Medical Director's attestation is handed to eSign as a human action. We found and documented two field-mapping bugs in the MCP server along the way.
