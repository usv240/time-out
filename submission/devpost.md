...
- **Foxit** runs the assembly agent: from a plain prompt, through the official Foxit MCP server, to a watermarked three-page record — then it stops. The Medical Director's attestation is a human eSign action, and it was signed: envelope `35704700`, status EXECUTED. The agent reads the outcome back but issues only GETs, so it can never act on an envelope. We found, worked around and reported upstream field-mapping bugs in two MCP tools, four mismatches in total, each with a reproduction.
